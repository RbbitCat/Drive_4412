...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	ret = GTP_GPIO_REQUEST(GTP_EN_PORT, "GTP_EN_IRQ");
    	if (ret < 0)
    	{
        	GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", (s32)GTP_EN_PORT, ret);
    	    	ret = -ENODEV;
    	}
    	else
    	{
		s3c_gpio_setpull(GTP_EN_PORT, S3C_GPIO_PULL_UP);
		GTP_GPIO_OUTPUT(GTP_EN_PORT, 1);
    	}
	GTP_GPIO_FREE(GTP_EN_PORT);


	ret = GTP_GPIO_REQUEST(GTP_RST_PORT, "GTP_RST_IRQ");
        if (ret < 0)
        {
        ...
        {
		s3c_gpio_setpull(GTP_RST_PORT, S3C_GPIO_PULL_UP);
                GTP_GPIO_OUTPUT(GTP_RST_PORT, 1);
        }
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	s3c_gpio_cfgpin(GTP_INT_PORT, S3C_GPIO_SFN(0xF));
        s3c_gpio_setpull(GTP_INT_PORT, S3C_GPIO_PULL_NONE);
	GTP_GPIO_FREE(GTP_RST_PORT);
        GTP_GPIO_FREE(GTP_INT_PORT);
